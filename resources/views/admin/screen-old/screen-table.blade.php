        ...
        <table class="table table-hover border" id="screen-display-table">
            ...
            @foreach ($data as $record)
                <tr data-id="{{ $record->id }}">
                    ...
                    <td class="text-center">{{ $record?->start_time?->format('h:i A') }} - {{ $record?->end_time?->format('h:i A') }}</td>
                </tr>
            @endforeach
            </tbody>
        </table>
        ...
        <div class="scroll-container">
            <div class="scroll-text text-white fw-bold">
                <img src="{{ asset('/assets-2/images/logo-screen/logo.png') }}" class="img-fluid " width="2.5%"
                     alt="">
                <span class="text-white fw-bold text-uppercase letter-spacing-1">{{ $data?->first()?->reference_session?->number }} REGULAR SESSION @ {{ $data?->first()->schedule?->venue }} | {{ $data?->first()->schedule?->first()?->date_and_time->format('F d, Y') }}<img
